...
app.post('/api/auth/register', async (req, res) => {
  ...
  try {
    await welcomeUser(username, email);
  } catch (e) {
...
app.post('/api/generate-certificate', async (req, res) => {
  ...
  try {
    ...
    doc.on('end', async () => {
      ...
      try {
        ...
        const transporter = nodemailer.createTransport({
          ...
          port: process.env.MAILGUN_SMTP_PORT
            ? parseInt(process.env.MAILGUN_SMTP_PORT)
            : 587,
          auth: {
        ...
        const from =
          ...
          `Fastpitch Quiz Team <${process.env.MAILGUN_SMTP_LOGIN}>`;

        // Load HTML template if available
        let htmlTemplate = `<p>Your certificate is ready, ${name}!</p>`;
        ...
        if (fs.existsSync(certTemplatePath)) {
          htmlTemplate = fs
            .readFileSync(certTemplatePath, 'utf8')
            .replace('{{username}}', name)
            .replace('{{score}}', String(score))
            .replace('{{total}}', String(total))
            .replace('{{certId}}', certId)
            .replace(
              '{{certificateUrl}}',
              (process.env.FRONTEND_URL || '') + '/certificate/' + certId
            );
        }
        ...
        const mailOptions = {
          from,
          to: email,
          subject: 'Your Certificate - Fastpitch Quiz',
          html: htmlTemplate,
          attachments: [
      ...
      try {
        await notifyTeacher(name, email, certUrl);
      } catch (e) {
